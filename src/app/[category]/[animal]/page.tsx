...
const AnimalProfilePage = ({ params }: { params: { category: string; animal: string } }) => {
  ...
  if (!animal || animal.category.toLowerCase() !== params.category.toLowerCase()) {
    notFound();
  }

  const breadcrumbs = [
    { href: '/', label: 'Home' },
    { href: `/${animal.category.toLowerCase()}`, label: animal.category },
    { href: `/${animal.category.toLowerCase()}/${animal.slug}`, label: animal.commonName },
  ];
            ...
            {related.length > 0 && (
                <Card>
                    ...
                    <CardContent>
                        ...
                        {related.map(r => (
                            <li key={r.slug}>
                                <Link href={`/${r.category.toLowerCase()}/${r.slug}`} className="flex items-center gap-2 group">
