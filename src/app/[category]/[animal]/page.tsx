...
import { animals, getAnimalBySlug, getAllAnimalParams } from '@/lib/animals';
...
export async function generateStaticParams() {
  return getAllAnimalParams();
}

export async function generateMetadata({ params }: { params: { animal: string } }) {
  const awaitedParams = await params;
  const animal = getAnimalBySlug(awaitedParams.animal);
  if (!animal) {
    return { title: 'Animal Not Found | Wildopedia' };
  }
  return {
...
const AnimalProfilePage = async ({ params }: { params: { category: string; animal: string } }) => {
  const awaitedParams = await params;
  const animal = getAnimalBySlug(awaitedParams.animal);

  // Validate category match using slugified comparison on BOTH sides
  if (!animal || toSlug(animal.category) !== toSlug(awaitedParams.category)) {
    notFound();
  }

  const breadcrumbs = [
    { href: '/', label: 'Home' },
    { href: `/${toSlug(animal.category)}`, label: animal.category },
    { href: `/${toSlug(animal.category)}/${toSlug(animal.slug)}`, label: animal.commonName },
  ];

  // Get full Animal objects whose slug appears in the relatedSpecies array
              ...
              <CardContent>
                <ul className="space-y-3 text-sm">
                  <li className="flex items-center gap-3">
                    ...
                    <span className="font-semibold">Conservation:</span> {animal.status.conservation}
                  </li>
                  <li className="flex items-center gap-3">
                    <Clock className="h-5 w-5 text-accent" />
                    <span className="font-semibold">Lifespan:</span> {animal.status.lifespan}
                  </li>
                  <li className="flex items-center gap-3">
                    <Weight className="h-5 w-5 text-accent" />
                    <span className="font-semibold">Weight:</span> {animal.status.weight}
                  </li>
                </ul>
              </CardContent>
            </Card>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {related.map(r => (
                      <li key={r.slug}>
                        <Link
                          href={`/${toSlug(r.category)}/${toSlug(r.slug)}`}
